...
import { isValidHex } from '../../src/Helpers/validators.js'
...
describe('Renderer', () => {
  ...
  describe('formatStats', () => {
    it('returns token count when elapsed is zero', () => {
      const formatStats = (renderer as unknown as Record<string, unknown>).formatStats as (
        tokens: number,
        elapsed: number
      ) => string
  ...
  describe('buildSuffix', () => {
    const getBuildSuffix = () =>
      ((renderer as unknown as Record<string, unknown>).buildSuffix as (mode: string) => string).bind(renderer)
  ...
  describe('getColor', () => {
    ...
    it('returns empty string for null TPS', () => {
      expect(getGetColor()(settings.getConfig(), null as unknown as number)).toBe('')
    })
