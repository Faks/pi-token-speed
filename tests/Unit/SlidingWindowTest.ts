...
import { COMPACTION_THRESHOLD, SlidingWindow as SlidingWindowConfig } from '../../src/Config/app.js'
import { SlidingWindow } from '../../src/Core/sliding-window.js'

describe('SlidingWindow', () => {
  const WindowMs = SlidingWindowConfig.Default

  it('returns 0 when empty', () => {
    const w = new SlidingWindow(WindowMs)
  ...
  it('compacts events after reaching threshold', () => {
    ...
    for (let i = 0; i < COMPACTION_THRESHOLD; i += 1) {
      w.record(1)
    }
  ...
  it('handles zero token records gracefully', () => {
    ...
    w.record(5)
    w.record(0)
    expect(w.getTps(now + SlidingWindowConfig.Min)).toBeGreaterThan(0)
  })
